...
def get_local_word2entity(entities):
    ...
    给定数据集中一行中的实体信息，构造一个从单词到实体索引的映射
     例如，给定实体='id_1：Harry Potter; id_2：England'，则返回 map= {'harry'：index_of（id_1），
     'potter'：index_of（id_1），'england'：index_of（id_2）}
...
def encoding_title(title, entities):
    ...
    根据word2index映射和entity2index映射编码标题
